...
- GPT Image 2.5 Sunburst (`openai/gpt-image-2.5-sunburst`, the precision tier) and GPT Image 2.5 Flare (`openai/gpt-image-2.5-flare`, the speed tier): both declare **no `resolution` parameter at all** — they size their own output from the aspect ratio — so the field is omitted and LocalInk's image-size choice has no effect for either model. Both take `quality`, with an enum reaching `xhigh` and `max`, wider than GPT Image 2's, which stopped at `high`; the app still sends the app-wide `"medium"`. Every aspect ratio except `4:5` and `5:4`.
...
`generatedImageModelRequiresZdrProvider` in `src/lib/generated-images.ts` decides which models get the field, via an explicit exemption set (`ZDR_EXEMPT_IMAGE_MODELS`) rather than an inference rule — a newly added model defaults to requiring ZDR unless it is added to the set. As of 2026-09-11, six models have zero ZDR-listed endpoints and are exempt: `openai/gpt-image-2.5-sunburst`, `openai/gpt-image-2.5-flare`, `qwen/qwen-image-3-pro`, `x-ai/grok-imagine-image-2.0`, `sourceful/riverflow-v2.5-pro`, and `black-forest-labs/flux.2-max`. The two GPT Image 2.5 tiers replaced `openai/gpt-image-2` and have zero ZDR-listed endpoints each, exactly like the model they replaced, so neither can pin a provider slug either (see below). Riverflow (provider `sourceful`) and FLUX.2 Max (provider `black-forest-labs/us-3`) are each served by a single provider that publishes no ZDR endpoint, so requesting ZDR would 404 rather than route them anywhere safer. The other five models (Nano Banana Pro, Nano Banana 2, Nano Banana 2 Lite, Seedream 5 Pro, Krea 2 Large) each have exactly one ZDR-listed endpoint, so ZDR routing leaves them no fallback to a non-ZDR provider — an outage at that single upstream now surfaces as a generation failure rather than being routed around. Check `https://openrouter.ai/api/v1/endpoints/zdr` before adding a model to decide which side of the exemption it belongs on.
...
- Instruction-tuned models (GPT Image 2.5 Sunburst, GPT Image 2.5 Flare, the Nano Banana models) get the negation-based prompt: style, subject, then an `Avoid:` list.
